...
public class FlowField {
	...
	private void randomize()
	{
		...
		for (int i = 0; i < cols; i++) {
			float yoff = 0;
			for (int j = 0; j < rows; j++) {
				float theta = PApplet.map(parent.noise(xoff, yoff, zoff), 0, 1, 0, PConstants.TWO_PI);
				PVector rand = new PVector((float) Math.cos(theta), (float) Math.sin(theta)); 
				rand.mult((float) 0.2);
				field[i][j].add(rand);
				yoff += 0.1;
			}
		...
		{ 
			for (int i = 0; i < cols; i++) {
				for (int j = 0; j < rows; j++) {
					...
					for (Zone z : zones)
					{
						PVector desired = PVector.sub(new PVector(i*cell, j*cell), z.origin);
